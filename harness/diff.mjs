...
// A-8: color is excluded from measurement FROM THE START. The palette is randomized at
// token-write time (A-7), so every ADAPTED section would otherwise carry a permanent color
// delta into STRUCT_THRESHOLD and eat the budget before geometry got a look in.
// Geometric and typographic fields stay; the non-color parts of shadows stay.
const WITH_COLOR = process.argv.includes('--with-color'); // debugging only, never gating
...
function structDiff(ref, ours) {
  ...
  // A-8: a shadow's geometry is measured, its color is not.
  const shadowGeom = (v) =>
    v == null ? null : (String(v).match(/-?\d+(\.\d+)?px/g) || []).join(' ');
  {
    const a = shadowGeom(get(ref, 'card.shadow')),
      b = shadowGeom(get(ours, 'card.shadow'));
    if (a != null && b != null) {
      const d = a === b ? 0 : 1;
      wsum += 1;
      dsum += d;
      if (d) worst.push(['card.shadow.geom', 100, a, b]);
    }
  }
...
function tokenDiff(ours) {
  ...
  if (ours.card) {
    ...
    if (ours.card.shadow) {
      const geom = (v) => (String(v).match(/-?\d+(\.\d+)?px/g) || []).join(' ');
      if (!TOKENS.shadow.some((t) => geom(t) === geom(ours.card.shadow)))
        viol.push(['card.shadow.geom', geom(ours.card.shadow)]);
    }
  }
...
for (const [route, def] of Object.entries(MAP.routes)) {
  ...
  for (const bp of BP_SET) {
    ...
    for (const s of def.sections) {
      ...
      const cls = s.class;
      // A-9: token conformance has no breakpoint dimension. Measure NOVEL once.
      if (cls === 'NOVEL' && bp !== 1440) continue;
